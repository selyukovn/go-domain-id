...
This package contains basic code for artificial domain `Id`s based on `Uint` and `UUID`-strings.

### Here's the thing:

Unlike, for example, the domain type `OrderNumber`,  
...
the domain type `Id` is most often based on a set dictated by data storage systems.
...
Often, domain objects require precisely such artificial identifiers due to the lack of natural ones.  
The code in this case will always be the same — i.e., duplicated.

A solution to the duplication problem is to move the domain `Id` into a domain `common` package as a shared type, like `Email`.  
...
but use it as a base for identifiers of different object types, not directly as a domain type.  
